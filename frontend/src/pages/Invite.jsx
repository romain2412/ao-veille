...
import { getInvitationInfo, acceptInvitation } from '../api/client'
...
export default function Invite() {
  ...
  useEffect(() => {
    getInvitationInfo(token)
      .then(setInfo)
      .catch(err => {
        setLoadError(err.response?.data?.detail || "Cette invitation n'est pas valide.")
      })
  }, [token])
    ...
    <div className="min-h-screen bg-fbgray flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-semibold text-brand-500">Création de votre compte</h1>
        ...
        {loadError && (
          <div className="text-center">
            ...
            <button onClick={() => navigate('/login')} className="mt-4 text-sm text-brand-500 font-semibold">
        ...
        {!loadError && !done && info && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {info.full_name && (
              <div>
                <label className="block text-sm font-semibold text-fbtext mb-1">Nom</label>
                <input
                  type="text"
